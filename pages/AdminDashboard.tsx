...
export const AdminDashboard = () => {
  ...
  const handleUserAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error('Erreur upload avatar vers bucket avatars:', error);
  ...
  const handleAIFill = async () => {
    if (!currentArticle.title) { alert("Veuillez saisir un titre d'abord pour guider World Canal Info AI."); return; }
    setIsProcessing(true);
    try {
        ...
        console.error('Erreur World Canal Info AI:', e);
        alert("Erreur World Canal Info AI. Vérifiez la configuration."); 
    }
    setIsProcessing(false);
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <div className="md:hidden bg-white text-brand-dark p-3 px-4 flex justify-between items-center sticky top-0 z-50 shadow-sm border-b border-gray-100">
          ...
          <div className="flex items-center gap-2">
              <Link 
                to="/" 
                className="flex items-center gap-1.5 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-brand-dark rounded-xl text-xs font-bold tracking-wider uppercase transition-all active:scale-95 border border-gray-200/80 shadow-sm"
                title="Voir le site public"
              >
                  <span className="text-sm">🌐</span>
                  <span className="hidden sm:inline text-[11px] font-black">Voir le site</span>
              </Link>
      ...
      {/* Sidebar */}
      <aside className={`w-72 bg-brand-dark text-white flex flex-col fixed h-full shadow-2xl z-40 overflow-y-auto transition-transform duration-300 ease-in-out md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:static md:h-screen pt-16 md:pt-0`}>
        ...
        <div className="p-4 border-b border-white/5">
            <Link 
              to="/" 
              onClick={() => setIsMobileMenuOpen(false)}
              className="flex items-center gap-3 px-5 py-3.5 bg-white/5 rounded-xl hover:bg-white/10 transition-all text-xs font-black uppercase tracking-widest text-brand-yellow border border-white/10 shadow-sm"
            >
                <span className="text-base">🌐</span> VOIR LE SITE
            </Link>
        </div>
                    ...
                    <div className="max-w-[850px] mx-auto space-y-4 sm:space-y-6">
                        ...
                        <div className="bg-white rounded-2xl shadow-2xs border border-gray-200 min-h-[450px] sm:min-h-[550px] md:min-h-[700px] flex flex-col relative overflow-hidden">
                            <style>{`
                                ...
                                .ql-snow .ql-picker { color: #4b5563 !important; }
                                .ql-snow .ql-picker-options { background-color: #ffffff !important; color: #1f2937 !important; border: 1px solid #e5e7eb !important; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1) !important; border-radius: 0.5rem; }
                                
                                /* Hover States */
                                .ql-snow .ql-picker:hover { color: #111827 !important; }
                                .ql-snow .ql-picker:hover .ql-picker-label { color: #111827 !important; }
                                .ql-snow .ql-picker-label:hover { color: #111827 !important; }
                                .ql-snow .ql-picker-item:hover { color: #d97706 !important; background-color: #fef3c7 !important; }
                                button.ql-active .ql-stroke { stroke: #d97706 !important; }
                                button.ql-active .ql-fill { fill: #d97706 !important; }
